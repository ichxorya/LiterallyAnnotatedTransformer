...
Checklist:
- Documentation: TODO Fully documented.
- Type-checking: TODO Passed (Mypy strict mode).
"""
...
class DirtyTransformer(nn.Module):
    ...
    @staticmethod
    def get_clones(
        module: nn.Module, N: int, keep_module: bool = True
    ) -> nn.ModuleList:
        """
        ...
        if keep_module and N >= 1:
            return nn.ModuleList(
                [module] + [copy.deepcopy(module) for i in range(N - 1)]
            )
    ...
    class Encoder(nn.Module):
        ...
        def __init__(
            ...
            self.encoder_layers = DirtyTransformer.get_clones(
                layers.Encoder(d_model, h, dropout_rate), N
            )
    ...
    class Decoder(nn.Module):
        ...
        def __init__(
            ...
            self.layers = DirtyTransformer.get_clones(
                layers.Decoder(d_model, h, dropout_rate), N
            )
